...
def main():
    ...
    if not ollama_available():
        write({'version':'1.1','generated_at':datetime.now(timezone.utc).isoformat(),'symbol':frozen.get('symbol'),'decision':'UNAVAILABLE','publish_authorized':False,'votes':[],'available_models':0,'disagreement':False,'provider':'ollama','reason':'Local Ollama runtime is not installed or not reachable; deterministic and Jev layers remain authoritative.'})
        return 0
    signal=load('signal_first_routing.json'); adv=load('adversarial_decision.json'); cf=load('counterfactual_analysis.json'); research=load('research_intelligence.json')
    ...
    for model in MODELS:
        try:
            ...
            except Exception: parsed={'decision':'REVIEW','confidence':0.0,'reasons':[raw[:500]],'parse_error':True}
