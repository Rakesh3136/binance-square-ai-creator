...
W,H=1600,900; UA='BinanceSquareAICreator/2.1 (Wikimedia Commons public-domain visual research)'; LICENSES=('CC0','Public domain')
...
def main():
    ...
    if photo is None:
        photo=Image.new('RGB',(W,H),(35,38,43)); d=ImageDraw.Draw(photo); d.text((80,90),f'${symbol()} — trader reality',font=font(64,True),fill='white'); d.text((80,190),'Original fallback illustration — market context preserved',font=font(34),fill='white'); source={'title':'original fallback illustration','license':'original','source':'Binance Square AI Creator'}
    ...
    sources=(history.get('sources') or [])[-8:]+[str(source.get('title') or source.get('url') or 'fallback')]; treatments_hist=(recent_treatments[-8:]+[treatment])[-8:]
    save(HISTORY,{'version':'2.1','updated_at':datetime.now(timezone.utc).isoformat(),'sources':sources[-8:],'treatments':treatments_hist,'policy':'Only public-domain or CC0 real-world photos; original captions; never copy famous meme images.'})
