...
def main():
    ...
    move = selected.get('price_change_24h')
    move_text = f'{float(move):+.1f}%' if isinstance(move, (int,float)) else 'another red candle'
    ...
    ax.text(0.55, 5.82, 'A completely original crypto-native reaction meme', fontsize=12, color='#cbd5e1', va='center')

    panels = [(0.45,3.15,3.55,2.15),(4.20,3.15,3.55,2.15),(7.95,3.15,3.55,2.15),(0.45,0.55,11.05,2.25)]
    captions = [
        'ME: “I WILL WAIT FOR CONFIRMATION.”',
        'THE CHART: one more candle…',
        f'ME: “OK, BUT WHAT ABOUT ${s}?”',
        f'PORTFOLIO THERAPY: {move_text}  •  I definitely meant to do that',
    ]
    for i, (x,y,w,h) in enumerate(panels):
        ...
        if i < 3:
            ...
            if i == 1:
                ax.plot([x+0.55,x+1.35,x+2.1,x+2.85],[y+1.35,y+1.65,y+0.8,y+0.55], linewidth=4)
        else:
    ...
    ax.text(11.45, 0.16, 'AI CREATOR • ORIGINAL VISUAL', fontsize=7.5, color='#64748b', ha='right')
