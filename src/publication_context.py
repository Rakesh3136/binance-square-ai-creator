...
def main():
    ...
    if selected_title:
        for article in news.get('articles') or []:
            ...
            if str(article.get('title') or '').strip()!=selected_title: continue
    ...
    if category=='comparison':
        for x in (director.get('primary_story') or {}).get('chart_symbols') or []:
            s=clean(x)
            if s and s not in requested: requested.append(s)
    # Primary asset is always first. Secondary charts are opt-in only for an
    # explicit comparison/pair request; never let a stale Brain symbol replace it.
    chart_symbols=[symbol]
    pair_requested=category=='comparison' or str((brain.get('visual_decision') or {}).get('layout') or '').lower()=='pair' or str((director.get('visual_plan') or {}).get('layout') or '').lower()=='pair'
    if pair_requested:
        for s in requested:
            if s!=symbol:
