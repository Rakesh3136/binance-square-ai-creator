...
def main():
    ...
    # Never invent BTC as a second chart. A second chart must come from verified story context.
    chart_symbols=[s for s in chart_symbols if s==symbol or s in {'XAUUSD','XAGUSD'} or any(clean(a.get('symbol'))==s for a in (news.get('articles') or []) if isinstance(a,dict))]
