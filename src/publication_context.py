...
def main():
    ...
    if category=='comparison':
        for x in (director.get('primary_story') or {}).get('chart_symbols') or []:
            s=clean(x)
            if s and s not in requested: requested.append(s)
    # Primary asset is always first. A second chart is opt-in only for an
    # explicit comparison or an explicit Brain pair decision.
