...
def main():
    ...
    for a in (news.get('articles') or news.get('items') or []):
        ...
        ns=news_score(a)
        if ns:news_stories.append({'lane':'breaking_news' if ns>=18 else 'news_market_impact','score':round(28+ns,2),'title':text(a.get('title'))[:200],'url':text(a.get('url') or a.get('link')),'reason':'fresh source contains a material catalyst'})
    ...
    if selected_symbol:
        ...
        if expfmt in ROTATION and (not news_stories or expfmt not in {'BREAKING NEWS + MARKET IMPACT','NEWS + CHART','MACRO + MARKET IMPACT'}): recommended=expfmt
    ...
    # Recent mix is a steering signal, never a claim about a hidden algorithm.
    ...
    brief={'generated_at':datetime.now(timezone.utc).isoformat(),'director_version':'4.2','algorithm_policy':'Use observed performance only as an experiment. Never claim knowledge of a private ranking algorithm.','primary_story':primary,'recommended_format':recommended,'authoritative_selection':selected,'news_context':news_stories[:10],'ranked_stories':stories[:40],'coverage_mix':coverage,'recent_format_context':recent_formats,'story_rules':['The preflight selected symbol is authoritative downstream.','Prefer a specific catalyst, surprise, level or measurable change over generic recaps.','Single-asset market posts require a TradingView chart; no custom panel may cover candles or the price area.','Creator calls become follow-ups only after fresh market data verifies the move; attribute the original source and never claim we predicted it.','Targets, stop/invalidation and 10x/20x scenarios must be conditional and evidence-based.','Never invent news, creator calls, prices, targets, volume or engagement.','One real question per post; no begging for likes/follows.','Rotate formats and narrative structures so consecutive posts do not feel templated.']}
    OUT.parent.mkdir(parents=True,exist_ok=True); OUT.write_text(json.dumps(brief,indent=2,ensure_ascii=False),encoding='utf-8')
    pre['content_director_4']=brief; pre['content_director_instruction']=f'Use authoritative asset ${selected_symbol} and format {recommended}; choose evidence over hype and rotate the narrative.'
    PREFLIGHT.write_text(json.dumps(pre,indent=2,ensure_ascii=False),encoding='utf-8')
    print(json.dumps({'status':'OK','director':'4.2','symbol':selected_symbol,'format':recommended,'ranked':len(stories)},indent=2))
