...
def main():
    ...
    category=text(selected.get('category')).lower(); recommended=FORMAT_BY_CATEGORY.get(category,'TRADINGVIEW CHART CHALLENGE')
    item=next((x for x in items if symbol(x.get('symbol'))==selected_symbol),None)
    ...
    if selected.get('news_title'):
        article_syms=[symbol(x) for x in (selected.get('news_symbols') or []) if valid(symbol(x))]
        if not article_syms:
            for a in news_stories:
                if a.get('title')==selected.get('news_title'):article_syms=[symbol(x) for x in a.get('symbols') if valid(symbol(x))];break
        related=article_syms
    ...
    brief={'generated_at':datetime.now(timezone.utc).isoformat(),'director_version':'4.2','algorithm_policy':'Use observed performance only as an experiment. Never claim knowledge of a private ranking algorithm.','primary_story':{**primary,'chart_symbols':chart_symbols},'recommended_format':recommended,'authoritative_selection':selected,'news_context':news_stories[:10],'ranked_stories':stories[:40],'coverage_mix':['breaking_news','news_market_impact','macro','technical_breakout','top_mover','volume_anomaly','liquidation','new_listing','creator_signal_outcome','education','comparison','follow_up','watchlist'],'visual_plan':{'provider':'TradingView','layout':'two_panel' if len(chart_symbols)>1 else 'single_panel','symbols':chart_symbols,'timeframe':'1H','custom_overlays':False},'story_rules':['Fresh material news outranks a generic mover.','The preflight selected symbol is authoritative.','News posts lead with the actual verified event and source.','When a second relevant asset exists, render two clean TradingView charts side by side in one attached image.','No custom panel may cover candles or price data.','Targets/SL/10x/20x are conditional scenarios only.','Never invent facts, sources, calls, prices, targets or outcomes.','Rotate hooks, formats and narrative structures.']}
