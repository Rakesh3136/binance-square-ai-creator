"""Creator 4.4 executive story director: select the strongest publishable story and learn from verified performance."""
...
def performance_bonus(story, feedback):
    prefs=feedback.get('learned_preferences') or {}; bonus=0.0; matches=[]
    category=story.get('lane',''); fmt=FORMAT_BY_CATEGORY.get(category,'TRADINGVIEW CHART CHALLENGE')
...
def main():
    market,news,pre,feedback=load(MARKET),load(NEWS),load(PREFLIGHT),load(FEEDBACK); selected=pre.get('selected_opportunity') or {}; selected_symbol=symbol(selected.get('symbol') or selected.get('topic')); items=collect_market(market); stories=[market_story(x) for x in items]
    news_stories=[]
