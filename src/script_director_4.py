"""Creator 5.11 story-specific writing contract.

Editorial structure only. Never invents market levels, trade outcomes, or copied
creator language. Public Square examples are used as pattern benchmarks, not
as templates or identities.
"""
from __future__ import annotations
import hashlib,json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
PREFLIGHT=ROOT/'data/live/editorial_preflight.json';FEEDBACK=ROOT/'data/intelligence/performance_feedback.json';FLOW=ROOT/'data/live/capital_flow_intelligence.json';STYLE=ROOT/'data/intelligence/square_style_intelligence.json';OUT=ROOT/'data/live/script_director_4.json'
ARCHETYPES={'event_context_impact':'newsroom','macro_chain':'macro_analyst','momentum_question':'high_energy','contrarian_risk':'contrarian','volatility_then_test':'risk_analyst','data_vs_price':'data_driven','price_discovery':'discovery','level_confirmation':'technical_analyst','call_result_next_test':'accountability','thesis_update':'update','compare_tradeoffs':'debate','one_chart_one_lesson':'teacher','what_to_watch':'conversational','capital_flow_long':'flow_trader','capital_flow_short':'flow_trader','capital_flow':'flow_trader','research_radar':'research_analyst','crypto_meme':'meme_creator'}
HOOKS={
...
'meme_creator':['Crypto is doing crypto things again.','Every trader knows this exact moment.','The chart has entered the part nobody puts in the trading plan.']}
...
def choose_question(category,symbol,selected,flow):
    cat=str(category or '').lower();s=clean(symbol);setup=selected.get('trade_setup') or {};side=str(setup.get('side') or '').upper()
    if cat in {'capital_flow_long','capital_flow_short'} or side in {'LONG','SHORT'}:
    ...
    if cat in {'breaking_news','news_and_macro','news'}:
        title=str(selected.get('news_title') or '').strip();return f'Does the next measurable market response support the thesis that this event matters for ${s}?' if title else f'What evidence would make this ${s} catalyst durable rather than temporary?'
    ...
    if cat=='crypto_meme':return f'Is ${s} the coin you are watching right now?'
    return f'What specific evidence would change your view on ${s} next?'

def main():
    p=load(PREFLIGHT);d=p.get('content_director_4') or {};selected=p.get('selected_opportunity') or {};feedback=load(FEEDBACK);flow=load(FLOW);style_intel=load(STYLE)
    ...
    elif category=='crypto_meme':narrative='crypto_meme';fmt='CRYPTO MEME'
    ...
    visual_format=((style_intel.get('benchmarks') or {}).get('format_mix') or {}).get(category,{}).get('visual')
    if not visual_format:
        visual_format='verified_chart' if category not in {'crypto_meme','community','commentary','education'} else 'original_meme' if category=='crypto_meme' else 'optional'
    directive={'version':'5.11','format':fmt,'primary_symbol':sym,'narrative_engine':narrative,'creator_archetype':style,'news_title':str(selected.get('news_title') or ''),'news_source':str(selected.get('news_source') or ''),'capital_flow_context':{'market_regime':(flow.get('market_rotation') or {}).get('market_regime'),'leaders':(flow.get('market_rotation') or {}).get('leaders',[]),'laggards':(flow.get('market_rotation') or {}).get('laggards',[]),'selected_trade_setup':trade_setup,'flow_confidence':selected.get('flow_confidence'),'relative_strength_to_btc':selected.get('relative_strength_to_btc'),'flow_proxy_score':selected.get('flow_proxy_score')},'performance_learning':{'preferred_style':learned_style or None,'style_sample':learned.get('sample',0),'preferred_hook_type':((feedback.get('learned_preferences') or {}).get('hook_type') or {}).get('prefer')},'square_benchmark':{'visual_format':visual_format,'platform_hook_window':'first_3_seconds','minimum_analysis_target_words':int(((style_intel.get('benchmarks') or {}).get('platform_guidance') or {}).get('minimum_analysis_target_words',50)),'learn_patterns_not_sentences':True},'hook_candidates':hooks,'question_candidates':[question],'question_id':q_hash,'writing_contract':['Write one finished Binance Square post, never an outline, briefing or instructions.','Make the selected opportunity the story. Do not switch to a different asset or unrelated fresh news.','The opening must contain a specific observation, tension, contradiction, relationship or decision — not generic market-check language.','The first two lines must earn the next paragraph: reveal a concrete tension or surprising relationship.','Use mobile-first paragraphs, varied rhythm and natural conversational language. Write like a sharp human crypto creator, not a research report.','Aim for at least 50 words of actual insight when the lane supports it; every sentence must add information, interpretation, personality or a decision.','Use a clear sequence: HOOK → NEW INFORMATION → INTERPRETATION → DECISION. Technical: HOOK → EVIDENCE → LEVEL/SCENARIO → QUESTION. News: EVENT → WHY IT MATTERS → MARKET RESPONSE → QUESTION. Flow: REGIME → RELATIVE STRENGTH → FLOW EVIDENCE → CONDITIONAL SETUP → INVALIDATION → QUESTION. Research: ANOMALY → EVIDENCE → MISSING DATA → WATCH CONDITION → QUESTION.','Every paragraph must add new information or a decision. Never paraphrase the headline as the body.','Use one or two strong supplied facts instead of a statistics dump.','Exactly one question, and it must be the supplied story-specific question at the end.','For NEWS: use verified event/source only and never infer BTC relevance without supplied evidence.','For TECHNICAL: use only supplied OHLCV levels; do not invent support/resistance, targets or stops.','For CAPITAL FLOW: only use the supplied conditional LONG/SHORT setup. Include trigger, invalidation and TP1/TP2 only when provided; state that they are scenario levels, not guarantees. If side is WAIT, do not create a trade call.','For CRYPTO MEME: make it genuinely funny, recognizable to crypto traders and tied to the supplied market/news context. Use the original visual package; do not copy a famous meme image or another creator’s joke. Do not invent a price move or claim a trade result. Keep it clearly entertainment/commentary, not investment advice.','Never write “will pump”, “will dump”, “guaranteed”, or a guaranteed target.','No artificial engagement, no copied wording and no creator imitation.','Do not force emojis; use them only when they improve the joke or readability.','Use exact supplied facts only.'],'retention_rules':['Do not start every post with ticker + percentage.','Do not reuse the same hook family in consecutive cycles when history is available.','Create a concrete reason to read the next paragraph.','When useful, create a return reason for a later outcome update.','For opportunity series, make the next update depend on a measurable outcome rather than empty “coming soon” bait.'],'anti_template_rules':['Reject “fresh check”, “quick market check”, “here is what matters”, “the next reaction matters”.','Reject generic CTA questions.','Reject a post whose only insight is that an asset went up/down.','Reject memes that are generic internet jokes with no crypto-native context.','Reject any paragraph that sounds like a reusable template with the asset name swapped.','Never copy a public creator’s sentence, signature phrase, meme or identity.'],'story_inputs':{'primary_story':story,'authoritative_selection':selected,'news_title':str(selected.get('news_title') or ''),'news_source':str(selected.get('news_source') or '')},'visual_symbols':visual_symbols,'recommended_visual_format':visual_format,'recommended_question':question}
    OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(directive,indent=2,ensure_ascii=False));p['script_director_4']=directive;p['content_director_instruction']=f'Use authoritative asset ${sym}, format {fmt}, narrative {narrative}, and only these visual symbols: {visual_symbols}.';PREFLIGHT.write_text(json.dumps(p,indent=2,ensure_ascii=False));print(json.dumps({'status':'OK','version':'5.11','format':fmt,'symbol':sym,'narrative_engine':narrative,'creator_archetype':style,'recommended_visual_format':visual_format,'question':question},indent=2,ensure_ascii=False))
