...
SCRIPTS = ['audience_intelligence.py','market_timing.py','experiment_engine.py','visual_intelligence.py','thesis_ledger.py','creator_evolution.py']
...
def main():
    ...
    report = {'generated_at': datetime.now(timezone.utc).isoformat(), 'status': 'OK' if not failures else 'DEGRADED', 'completed': results, 'failures': failures, 'brain_deferred_to_production_stage': True, 'rule': 'Creator Brain runs after preflight and engagement selection.'}
