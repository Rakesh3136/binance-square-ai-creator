...
def main() -> None:
    ...
    symbol = re.sub(r"[^A-Z0-9]", "", str(payload.get("symbol") or context.get("symbol") or "").upper().replace("USDT", ""))
    category = str(context.get("category") or payload.get("category") or "").lower().strip()
    cashtag = f"${symbol}" if symbol else ""
    has_cashtag = bool(symbol and re.search(rf"(?<![A-Za-z0-9_])\${re.escape(symbol)}(?![A-Za-z0-9_])", text, flags=re.I))
    ...
    has_verified_widget = isinstance(widget_markers, dict) and bool(
        widget_markers.get("verified") is True and (widget_markers.get("symbol") or widget_markers.get("id"))
    )
