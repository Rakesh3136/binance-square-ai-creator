...
def add_market_candidate(pool, category, item, boost=0.0):
    ...
    if category in {"top_gainers", "top_losers"}:
        ...
        raw = min(100.0, raw + max(0.0, 25.0 - float(item.get("days_since_listing") or 30) * 3.0))
...
def main():
    ...
    pool = []
    for item in rows(market.get("top_gainers"))[:10]:
        add_market_candidate(pool, "top_gainers", item)
    for item in rows(market.get("top_losers"))[:10]:
        add_market_candidate(pool, "top_losers", item)
    for item in rows(market.get("highest_volume"))[:10]:
        add_market_candidate(pool, "volume_leaders", item)
    for item in rows(market.get("new_listing_market"))[:10]:
        add_market_candidate(pool, "new_listings", item, 5)

    signals = [x for x in rows(market.get("top_content_signals")) if isinstance(x, dict)]
    for item in sorted(signals, key=lambda x: float(x.get("intraday_range_percent") or 0), reverse=True)[:8]:
        add_market_candidate(pool, "high_volatility", item, 3)
    for item in [x for x in signals if str(x.get("symbol") or "") in {"BTCUSDT", "ETHUSDT"}]:
        add_market_candidate(pool, "technical_setup", item, 4)
    ...
    for c in pool:
        ...
        recent = publication_counts.get(base, 0)
        mem = memory_counts.get(base, 0)
        cat = category_counts.get(c["category"], 0)
        cooldown = last_asset_time.get(base)
        active = bool(cooldown and now - cooldown < timedelta(hours=ASSET_COOLDOWN_HOURS))
        c.update({
            "recent_count": recent,
            "memory_count": mem,
            "category_recent_count": cat,
            "cooldown_active": active,
            "adjusted_score": round(
                c["raw_score"] - min(55, recent * 22) - min(54, mem * MEMORY_REPEAT_PENALTY) - min(35, cat * CATEGORY_REPEAT_PENALTY) - (45 if active else 0),
                2
            ),
            "repeated": recent >= 2 or mem >= MEMORY_HARD_BLOCK_COUNT or active
        })
    ...
    portfolio_lane = str(portfolio.get("selected_lane") or "").lower()
    feedback_rows = rows(feedback.get("ranked_experiments"))
    best_experiment = (feedback_rows[0].get("experiment") if feedback_rows and isinstance(feedback_rows[0], dict) else None)
    ...
    if portfolio_lane:
        ...
        if lane_candidates:
            eligible = lane_candidates + [c for c in eligible if c not in lane_candidates]
    ...
    if best:
        selected = {
            ...
            "instruction": f"Use {best['category'].replace('_', ' ')} as the starting lane. Prefer a fresh, materially different asset and angle. Do not repeat recent coverage without a major verified event."
        }
