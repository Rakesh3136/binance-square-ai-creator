...
MARKET=Path('data/live/market_snapshot.json');NEWS=Path('data/live/news_snapshot.json');MEMORY=Path('analytics/strategy_memory.json');PUBLICATIONS=Path('analytics/publication_log.jsonl');FEEDBACK=Path('data/live/feedback_strategy.json');PORTFOLIO=Path('data/live/content_portfolio.json');OUTPUT=Path('data/live/editorial_preflight.json')
MIN_MARKET_SCORE=72.0;NEWS_FRESH_MINUTES=180;NEWS_MATERIAL_SCORE=55.0;MEMORY_REPEAT_PENALTY=18.0;MEMORY_HARD_BLOCK_COUNT=3;CATEGORY_REPEAT_PENALTY=14.0;ASSET_COOLDOWN_HOURS=12

def load(p):
    if not p.exists():return {}
    try:x=json.loads(p.read_text(encoding='utf-8'));return x if isinstance(x,dict) else {}
    except Exception:return {}
def rows(v):return v if isinstance(v,list) else []
def extract_symbols(t):
    found=set(); blob=str(t or '').upper()
    for token in re.findall(r'(?<![A-Z0-9])\$?[A-Z][A-Z0-9]{1,11}(?:USDT)?(?![A-Z0-9])',blob):
        b=token.replace('$','');b=b[:-4] if b.endswith('USDT') else b
        if 2<=len(b)<=10:found.add(b)
...
def main():
    market,news,memory,feedback=load(MARKET),load(NEWS),load(MEMORY),load(FEEDBACK);pubs=recent_publications();pc=Counter();cc=Counter();last={}
    for r in pubs:
    ...
    if best:
        selected={'category':best['category'],'symbol':best['topic'],'reason':best['reason'],'recommended_experiment':None,'instruction':'Lead with the verified news event, explain why it matters now, then connect it to market impact. Never turn fresh news into a generic price recap.' if best.get('type')=='news' else f"Use {best['category'].replace('_',' ')} as the starting lane.",}
