...
Adds a bounded audience-learning adjustment from Creator 22.0 and protects
verified prediction outcomes as an accountability lane. Historical performance
is observational only: it can increase/decrease testing priority, but it cannot
establish causality, invent metrics, or override hard editorial/publication
safety rules.
"""
...
def proof_selection(proof):
    ...
    return {
        'category':'creator_signal_outcome','lane':'accountability','symbol':symbol,
        'score':100.0,'ranker_score':100.0,
        'reason':'Verified prior prediction outcome is available for transparent public proof.',
        'proof_status':proof.get('status'),'proof_type':proof.get('proof_type'),
        'call_id':proof.get('call_id'),'post_id':proof.get('post_id'),
        'direction':proof.get('direction'),'reference_price':proof.get('reference_price'),
        ...
        'content_brief':proof.get('content_brief'),'next_hook':proof.get('next_hook'),
        'chartable':True,
    }

def main():
    brief=load(BRIEF);intel=load(INTEL);pref=load(PREF);audience=load(AUDIENCE);proof=load(PROOF)
    proof_choice=proof_selection(proof)
    ranked=[]
    ...
    chosen=ranked[0] if ranked else {};current=pref.get('selected_opportunity') or {};current_category=str(current.get('category') or '').lower();manual=bool(pref.get('manual_topic')) or bool(current.get('manual_topic'))
    ...
    if proof_choice:
        ...
        reason='Creator 6.3 evidence ranker + bounded Creator 22.1 audience preference selected the strongest eligible opportunity.';manual_or_protected=False
    ...
    pref['opportunity_ranking_6']={'version':'6.3+22.1','generated_at':datetime.now(timezone.utc).isoformat(),'selection_reason':reason,'manual_or_protected':manual_or_protected,'audience_learning_used':bool(audience.get('observed_patterns')),'proof_priority_used':bool(proof_choice),'top_candidates':ranked[:15]}
    pref['content_director_instruction']='Creator 6.3 ranker + Creator 22.1 bounded audience learning is authoritative. Verified prediction proof is protected when available. Use the selected opportunity exactly; do not substitute another asset or story. Never use BTC as an arbitrary news-chart proxy.'
    OUT.parent.mkdir(parents=True,exist_ok=True)
    OUT.write_text(json.dumps({'version':'6.3+22.1','generated_at':datetime.now(timezone.utc).isoformat(),'selected':chosen,'top_candidates':ranked[:25],'audience_learning':{'used':bool(audience.get('observed_patterns')),'source':'creator_22_0_audience_board.json','causal_claims_allowed':False,'max_adjustment':6.0},'proof_priority':{'used':bool(proof_choice),'source':'public_prediction_proof.json','only_terminal_win_allowed':True},'policy':['Market/news candidates compete on one score.','Audience learning only changes bounded testing preference from repeated explicit performance.','Audience observations never establish causality.','Verified prediction outcomes are published only as transparent proof when a terminal WIN exists.','News without an evidence-backed asset symbol cannot become the primary TradingView story.','Funding/OI/orderbook observations confirm rather than independently create a story.','Manual topics and verified creator outcomes/follow-ups remain protected.','No missing evidence is invented.']},indent=2,ensure_ascii=False),encoding='utf-8')
    PREF.write_text(json.dumps(pref,indent=2,ensure_ascii=False),encoding='utf-8')
    print(json.dumps({'status':'OK','version':'6.3+22.1','selected_category':chosen.get('category'),'selected_symbol':chosen.get('symbol'),'selected_news':chosen.get('news_title'),'ranked_candidates':len(ranked),'ranker_score':chosen.get('ranker_score'),'proof_priority_used':bool(proof_choice),'audience_learning_used':bool(audience.get('observed_patterns'))},indent=2))
