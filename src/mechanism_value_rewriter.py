...
def generic_bridge(s):return any(x in norm(s).lower() for x in GENERIC_BRIDGES)
...
def recent_repetitions(text):
    ...
    try:
        for row in PUBLICATION_LOG.read_text(encoding='utf-8').splitlines()[-12:]:
            ...
            old=str(o.get('text') or o.get('post') or o.get('content') or '');oldset={re.sub(r'[^a-z0-9 ]','',s.lower()).strip() for s in sentences(old) if len(s.split())>=8};matches.extend(sorted(cur&oldset))
    except Exception:pass
    return matches[:8]
def deterministic_bridges(original):
    symbol=(re.findall(r'\$[A-Z][A-Z0-9]{1,14}\b',original) or ['$THIS-ASSET'])[0];pct=(re.findall(r'\b[+-]?\d+(?:\.\d+)%',original) or ['the observed move'])[0];fact=next((s for s in sentences(original) if pct in s or symbol in s),'')
    if fact:
        return [f'For {symbol}, that matters because the existing {pct} move is the market fact behind the reaction, while the next response shows whether traders are accepting or rejecting it.',f'The useful signal in {symbol} is what happens after the observed {pct} move: follow-through would support the reaction, while rejection would weaken it.',f'{symbol} is worth watching because the observed {pct} move creates a test between follow-through and rejection rather than proving either outcome in advance.']
    return [f'For {symbol}, the next observable reaction is what separates a temporary burst of attention from a meaningful change in behavior.',f'The market read on {symbol} depends on the next observable response, because that is where attention either turns into follow-through or fades.',f'What matters next for {symbol} is the reaction itself, because a move becomes more informative when the market confirms or rejects it.']
def main():
    ...
    original_question=qs[0].strip();body_without_question=original.replace(original_question,'').rstrip();reps=recent_repetitions(body_without_question);bridge=''
