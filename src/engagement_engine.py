...
def main():
    ...
    if untested:
        ...
        scores = {x["id"]: exp_score(x) for x in EXPERIMENTS}
        max_score = max(scores.values(), default=0.0)

        if max_score > 0.0001:
            exp = max(EXPERIMENTS, key=lambda x: scores[x["id"]])
        else:
            exp = min(EXPERIMENTS, key=lambda x: (exp_count.get(x["id"], 0), last_exp_index.get(x["id"], -1)))
    ...
    strategy = {
        ...
        "monetization": "use relevant cashtag ($SYMBOL) naturally in top lines for Write to Earn attribution; never promise earnings",
    ...
    selected["instruction"] = (
        ...
        f"Keep it short, visual-first and conversational. Do not repeat recently covered assets without a major verified event. "
        f"Do not reuse the previous editorial style or opening cadence."
    )
    ...
    result = {
        ...
        "learning_note": "Observed reply/follower rates influence the next experiment after enough evidence exists. Least recently tested experiments are rotated when scores are tied."
    }
