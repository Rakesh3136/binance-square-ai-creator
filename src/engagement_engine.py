...
def main():
    ...
    if untested:
        ...
        # Prefer under-sampled formats (<3 samples) to establish balanced baselines
        min_samples = min(exp_count.get(x["id"], 0) for x in EXPERIMENTS)
        if min_samples < 3:
            exp = min(EXPERIMENTS, key=lambda x: (exp_count.get(x["id"], 0), -exp_score(x)))
        else:
            exp = max(EXPERIMENTS, key=lambda x: (exp_score(x), -exp_count.get(x["id"], 0)))
