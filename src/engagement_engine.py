...
Chooses a content lane and writing structure from evidence instead of letting the
creator fall into a single repeated script. It optimizes for genuine replies,
follower conversion and useful cashtag/chart interaction; it never fabricates
...
EXPERIMENTS=[
 {"id":"A","format":"BREAKING NEWS + MARKET IMPACT","hook":"the event changed the setup","question":"Bullish, bearish, or wait?"},
 {"id":"B","format":"TRADINGVIEW CHART CHALLENGE","hook":"one level decides the setup","question":"Breakout or fakeout?"},
 {"id":"C","format":"COIN VS COIN","hook":"two assets are diverging","question":"Which one has the stronger setup?"},
 {"id":"D","format":"DATA SURPRISE","hook":"one number traders may miss","question":"Did you notice this signal?"},
 {"id":"E","format":"BREAKOUT / FAKEOUT","hook":"price is testing a decisive level","question":"Would you wait for confirmation?"},
 {"id":"F","format":"NEWS + CHART","hook":"the headline meets the price action","question":"Does the chart confirm the news?"},
 {"id":"G","format":"LIQUIDATION STORY","hook":"a sharp flush changed positioning","question":"Reversal or continuation?"},
 {"id":"H","format":"TOP MOVERS","hook":"today's strongest move has a reason","question":"Chase, fade, or wait?"},
 {"id":"I","format":"CREATOR CALL OUTCOME","hook":"a previous public call can now be measured","question":"Was the original thesis right?"},
 {"id":"J","format":"FOLLOW-UP / UPDATE","hook":"the setup changed after the last post","question":"Has the invalidation level changed?"},
 {"id":"K","format":"EDUCATION FROM LIVE CHART","hook":"one chart pattern explained with real data","question":"Would you trade this pattern?"},
]
...
def main():
    ...
    ranked.sort(key=lambda x:x["engagement_score"],reverse=True); selected=ranked[0] if ranked else (pre.get("best_market_candidate") if isinstance(pre.get("best_market_candidate"),dict) else {})
    tested={e for e in exp_count if e}; untested=next((x for x in EXPERIMENTS if x["id"] not in tested),None)
    if untested: exp=untested
    else:
        def exp_score(x):
            rs=rate(rows_by_exp[x["id"]],"replies"); fs=rate(rows_by_exp[x["id"]],"followers_gained"); ls=rate(rows_by_exp[x["id"]],"likes"); ss=rate(rows_by_exp[x["id"]],"shares"); n=len(rows_by_exp[x["id"]]); conf=min(1.0,n/10)
            return (0.50*(rs or 0)+0.30*(fs or 0)+0.12*(ls or 0)+0.08*(ss or 0))*conf
        exp=max(EXPERIMENTS,key=exp_score)
    last_style=str(history[-1].get("editorial_style") or "").upper() if history else ""
    previous_style=str(history[-2].get("editorial_style") or "").upper() if len(history)>1 else ""
    forbidden={last_style,previous_style}
    ...
    strategy={
      "primary_goal":"maximize genuine interaction, follower conversion and useful reader actions; quality beats posting volume",
      "experiment_id":exp["id"],"experiment":exp,
      "content_coverage":["macro/Fed/inflation/rates","ETF/regulation","security hacks/exploits","major listings/unlocks/upgrades","BTC/ETH/BNB market structure","altcoin leaders/laggards","volume and volatility anomalies","liquidations","new narratives","creator call outcomes","educational chart breakdowns"],
      "hook_rule":"first 1-2 lines must contain a concrete event, number, level, divergence or surprising observation; never generic introductions",
      "question_rule":"one low-friction question only; prefer A/B, breakout/fakeout, level choice or thesis confirmation",
      "style_rule":"Never repeat the same editorial style, opening cadence, emoji pattern or paragraph structure in consecutive posts. Rotate formats based on evidence and recent history.",
      "writing_rule":"Every post must explain why the information matters, what the chart/news says now, and what would invalidate the thesis when technically supported.",
      "technical_rule":"Single-asset technical stories require the verified TradingView chart and synchronized current price/support/resistance/target/invalidation values. Never invent a level.",
      "creator_tracking_rule":"A creator call becomes an outcome story only after fresh market data verifies the move; measure the result and do not rewrite history.",
      "growth_rule":"Track views, likes, comments, shares and post-attributed follower gains separately. Never convert missing metrics to zero.",
      "monetization_rule":"Use the relevant $cashtag and chart naturally because eligible Square activity can be attributed, but never promise earnings or returns.",
