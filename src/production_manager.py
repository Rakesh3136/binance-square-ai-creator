"""Final publication authority for Creator 5.1.

The frozen publication context is the single source of truth for the primary asset.
Accepts a single TradingView chart or a clean TradingView pair image, and lets the
editorial brain choose image-post vs article mode.
"""
...
def choose_mode():
    ctx=load(CONTEXT_PATH); mode=str(ctx.get('publication_mode') or '').lower()
...
def main():
    ...
    print('Production manager 5.1: normal gate rejected; running one bounded rescue.')
