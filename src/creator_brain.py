...
def main():
    ...
    # The selected opportunity is authoritative. News can no longer silently
    # override a stronger flow/research/technical lane after preflight ranking.
    lane_map={
    ...
    if not engine:
        # Only use NEWSROOM for an unclassified opportunity when the selected story
        # itself is news-backed; never let unrelated fresh news hijack the lane.
    ...
    visual_decision=dict(visual or {});chart_first=next_format in {'CHART BREAKDOWN','BREAKOUT/FAKEOUT','TARGET MAP','NEWS + CHART','CREATOR CALL OUTCOME','MOMENTUM + FOLLOW-THROUGH','CAPITAL FLOW TRADE SETUP'} or engine in {'TECHNICAL_SETUP','CREATOR_CALL_ACCOUNTABILITY','CAPITAL_FLOW_LONG','CAPITAL_FLOW_SHORT'}
    ...
    else:visual_decision.update({'required':True,'provider':'TradingView','type':'candlestick_chart','reason':'Use a real market chart when it adds evidence; never use decorative graphics'})
