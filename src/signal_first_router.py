    ...
    for source in (brief.get('ranked_stories'),pre.get('ranked_stories')):
        if isinstance(source,list):
            for x in source:
                ...
                score=num(x.get('score'),num(x.get('ranker_score')))
                if score>=MIN_SCORE:(primary if lane(x) in PRIMARY_LANES or x.get('type')=='flow' else market_candidates).append({**x,'score':score})
...
def main():
    pre=load(PREFLIGHT); brief=load(DIRECTOR); cad=load(CADENCE); market=load(MARKET); ranking=load(RANKING); rows=recent(); primary,markets=candidates(brief,pre,cad,market,ranking)
