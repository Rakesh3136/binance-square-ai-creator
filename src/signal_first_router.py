...
def signal_candidate(brief, preflight):
    ...
    for item in ranked:
        ...
        if (l in PRIMARY_LANES or l in MARKET_SIGNAL_LANES or item.get('type') in {'flow', 'market'}) and score >= MIN_PRIMARY_SCORE and item.get('symbol'):
...
def main():
    ...
    if candidate and cadence_publish:
        ...
        if publish:
            # Bind the primary decision into the existing authoritative preflight
            # contract so downstream freeze/brain stages cannot silently revert to a meme.
            bound = dict(selected)
            bound['category'] = bound.get('category') or ('capital_flow_long' if str(bound.get('flow_side','')).upper() == 'LONG' else 'capital_flow_short' if str(bound.get('flow_side','')).upper() == 'SHORT' else 'top_gainers')
            bound['lane'] = bound.get('lane') or bound.get('type')
            bound['symbol'] = str(bound.get('symbol') or '').upper()
            bound['score'] = num(bound.get('score'))
            bound['signal_first_primary'] = True
    ...
    if not publish and cadence_publish and existing_category in MEME_LANES:
        ...
        if share < MEME_MAX_SHARE:
            ...
            selected = {'category': 'crypto_meme', 'lane': 'meme', 'score': 0, 'symbol': '', 'primary_signal_available': False, 'meme_share_before': round(share, 3)}
        else:
            decision = 'MEME_QUOTA_BLOCKED'
            reason = f'meme_share_limit_reached ({memes}/{total} >= {MEME_MAX_SHARE:.0%})'
    ...
    result = {
        ...
        'router_version': '1.1-signal-first',
        'publish': publish,
        'decision': decision,
        'reason': reason,
        ...
        'policy': {
            ...
            'meme_quota_is_rolling_20_publications': True,
