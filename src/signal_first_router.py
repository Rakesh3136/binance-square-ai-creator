...
Market opportunities selected by the authoritative cadence/director are a
secondary market-signal lane and must not be silently discarded just because
they are not represented in ranked_stories. Every true flow prediction still
requires direction, trigger, TP1/TP2 and SL/invalidation.
"""
...
MARKET_SIGNAL_LANES={'market','top_gainers','top_losers','high_volatility','volume_leaders','technical_setup','next_gainer_candidate','next_loser_candidate'}
...
def recent_rows():
    ...
    accepted={'PUBLISHED_AUTONOMOUSLY','PUBLISHED_VERIFIED_BY_API_RESPONSE','VERIFIED_PUBLISHED','PUBLISHED_SUBMITTED_504'}
    rows=[]
    for line in PUBLICATIONS.read_text(encoding='utf-8').splitlines()[-120:]:
        try:
            r=json.loads(line)
            if isinstance(r,dict) and r.get('status') in accepted:rows.append(r)
...
def thesis_key(item):
    ...
    setup=setup if isinstance(setup,dict) else {}; pred=pred if isinstance(pred,dict) else {}
...
def repetition_blocked(item,rows):
    symbol=str(item.get('symbol') or '').upper(); key=thesis_key(item); setup=item.get('trade_setup') or {}; setup=setup if isinstance(setup,dict) else {}; pred=item.get('prediction') or {}; pred=pred if isinstance(pred,dict) else {}; side=str(setup.get('side') or pred.get('direction') or '').upper()
    for r in rows[-5:]:
...
def flow_is_complete(item):
    setup=item.get('trade_setup') or {}; pred=item.get('prediction') or {}; setup=setup if isinstance(setup,dict) else {}; pred=pred if isinstance(pred,dict) else {}
...
def add_candidate(target,item):
    if not isinstance(item,dict) or not item.get('symbol'):return
    score=num(item.get('score'),num(item.get('ranker_score')))
    if score<MIN_PRIMARY_SCORE:return
    target.append({**item,'score':score})

def signal_candidates(brief,preflight,cadence):
    primary=[]; secondary=[]
    ...
    if isinstance(selected,dict):
        l=lane(selected)
        (primary if l in PRIMARY_LANES or selected.get('type')=='flow' else secondary).append({**selected,'score':num(selected.get('score'),num(selected.get('ranker_score')))}) if selected.get('symbol') and num(selected.get('score'),num(selected.get('ranker_score')))>=MIN_PRIMARY_SCORE else None
    # Critical recovery path: cadence is authoritative about the opportunity
    # selected for this cycle. Do not drop it merely because the director's
    # ranked_stories list omitted the same object.
    cadence_selected={'symbol':cadence.get('selected_symbol'),'category':cadence.get('selected_category'),'score':num(cadence.get('effective_score'),num(cadence.get('ranker_score'))),'type':'market','reason':'authoritative cadence selection'}
    if cadence_selected.get('symbol') and cadence_selected.get('score',0)>=MIN_PRIMARY_SCORE:
        l=lane(cadence_selected)
        (primary if l in PRIMARY_LANES else secondary).append(cadence_selected)
    # Also accept the director's primary story when it is the only surviving
    # representation of a qualified market opportunity.
    primary_story=brief.get('primary_story') or {}
    if isinstance(primary_story,dict) and primary_story.get('symbol'):
        add_candidate(primary if lane(primary_story) in PRIMARY_LANES or primary_story.get('type')=='flow' else secondary,primary_story)
...
def main():
    pre=load(PREFLIGHT); brief=load(DIRECTOR); cadence=load(CADENCE); rows=recent_rows(); primary_candidates,secondary_candidates=signal_candidates(brief,pre,cadence)
    ...
    if candidate and cadence_publish:
        selected=dict(candidate); primary=lane(candidate) in PRIMARY_LANES or candidate.get('type')=='flow'; publish=True; decision='PRIMARY_SIGNAL' if primary else 'MARKET_SIGNAL'; reason='qualified_non_repetitive_signal_selected'
        if primary and not flow_is_complete(candidate):publish=False;primary=False;decision='NO_PUBLISH';reason='prediction_contract_missing_direction_trigger_tp_or_sl'
        if publish:
            bound=dict(selected); setup=bound.get('trade_setup') or {}; pred=bound.get('prediction') or {}; setup=setup if isinstance(setup,dict) else {}; pred=pred if isinstance(pred,dict) else {}; side=str(setup.get('side') or pred.get('direction') or '').upper(); bound['category']=bound.get('category') or ('capital_flow_long' if side=='LONG' else 'capital_flow_short' if side=='SHORT' else bound.get('category') or 'market'); bound['lane']=bound.get('lane') or bound.get('type') or bound.get('category'); bound['symbol']=str(bound.get('symbol') or '').upper(); bound['score']=num(bound.get('score')); bound['signal_first_primary']=primary; bound['prediction_contract_complete']=flow_is_complete(bound) if primary else False; bound['thesis_key']=thesis_key(bound); pre['selected_opportunity']=bound; pre['signal_first_routing']={'decision':decision,'primary':primary,'bound_symbol':bound['symbol'],'bound_category':bound['category'],'prediction_contract_complete':bound['prediction_contract_complete'],'prediction':pred or {'direction':side,'entry_trigger':setup.get('trigger'),'tp1':setup.get('tp1'),'tp2':setup.get('tp2'),'sl':setup.get('invalidation')}}; PREFLIGHT.write_text(json.dumps(pre,indent=2,ensure_ascii=False),encoding='utf-8')
    ...
    result={'generated_at':datetime.now(timezone.utc).isoformat(),'router_version':'1.4-authoritative-cadence-recovery','publish':publish,'decision':decision,'reason':reason,'primary_signal':primary,'secondary_meme':meme,'selected':selected,'cadence_publish':cadence_publish,'existing_category':existing_category,'blocked_candidates':blocked,'candidate_counts':{'primary':len(primary_candidates),'market':len(secondary_candidates)},'policy':{'primary_lane':'capital_flow_and_measurable_market_outcomes','secondary_lane':'controlled_market_signals_and_memes','primary_priority_order':['verified_outcome','capital_flow','market_signal'],'minimum_primary_score':MIN_PRIMARY_SCORE,'minimum_flow_confidence':MIN_FLOW_CONFIDENCE,'prediction_required':['direction','entry_trigger','tp1','tp2','sl','confidence'],'target_meme_share_max':MEME_MAX_SHARE,'meme_cannot_displace_qualified_signal':True,'meme_quota_is_rolling_20_publications':True,'no_signal_means_no_signal_post':True,'anti_repetition_window':5,'text_similarity_block':TEXT_SIMILARITY_BLOCK,'cadence_selected_recovery':True,'verified_publication_statuses_counted':True}}
