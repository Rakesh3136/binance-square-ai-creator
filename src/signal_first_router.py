...
PREFLIGHT=ROOT/'data/live/editorial_preflight.json'; DIRECTOR=ROOT/'data/live/content_director_brief.json'; CADENCE=ROOT/'data/live/autonomous_cadence_6.json'; MARKET=ROOT/'data/live/market_snapshot.json'; FLOW=ROOT/'data/live/capital_flow_intelligence.json'; RANKING=ROOT/'data/live/opportunity_ranking_6.json'; PUBLICATIONS=ROOT/'analytics/publication_log.jsonl'; OUT=ROOT/'data/live/signal_first_routing.json'
...
def setup_parts(x):
    ...
    side=str(s.get('side') or p.get('direction') or x.get('flow_side') or '').upper(); tr=s.get('trigger',p.get('entry_trigger')); tp1=s.get('tp1',p.get('tp1')); tp2=s.get('tp2',p.get('tp2')); sl=s.get('invalidation',p.get('sl')); conf=num(x.get('flow_confidence'),num((x.get('multitimeframe') or {}).get('confidence'),num(x.get('score'))))
...
def derive(candidate,market):
    ...
    conf=max(MIN_FLOW_CONF,min(95.0,num(candidate.get('score'),num(candidate.get('ranker_score')))))
...
def candidates(brief,pre,cad,market,flow_data,ranking):
    ...
    # Directly bind the fresh capital-flow engine too. This keeps Signal-First
    # independent of editorial score weighting and prevents a top-gainer fallback
    # from replacing a complete conditional flow setup.
    for x in (flow_data.get('top_conditional_setups') or []):
        if isinstance(x,dict) and flow_complete(x):
            primary.append({**x,'type':'flow','lane':x.get('lane') or ('capital_flow_long' if str((x.get('prediction') or {}).get('direction') or (x.get('trade_setup') or {}).get('side') or '').upper()=='LONG' else 'capital_flow_short'),'category':x.get('category') or ('capital_flow_long' if str((x.get('prediction') or {}).get('direction') or (x.get('trade_setup') or {}).get('side') or '').upper()=='LONG' else 'capital_flow_short'),'score':num(x.get('flow_score'),x.get('flow_confidence'))})

    # opportunity_ranking_6 stores the authoritative choice under `selected` and
    # the viable alternatives under `top_candidates`. The previous router looked
    # for selected_symbol at the root, so it silently ignored the entire ranking.
    ranking_items=[]
    ...
    # cadence can expose a direct selected_symbol in some historical versions.
    ...
    return (sorted(primary,key=lambda x:(1 if flow_complete(x) else 0,num(x.get('flow_confidence'),0),num(x.get('score')),),reverse=True),
            sorted(market_candidates,key=lambda x:num(x.get('score')),reverse=True))
...
def main():
    pre=load(PREFLIGHT); brief=load(DIRECTOR); cad=load(CADENCE); market=load(MARKET); flow=load(FLOW); ranking=load(RANKING); rows=recent(); primary,markets=candidates(brief,pre,cad,market,flow,ranking)
