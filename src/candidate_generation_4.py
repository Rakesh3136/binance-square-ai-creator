"""Creator 5.1 candidate-generation brief.
...
def main():
    ...
    fmt=d.get('recommended_format','TOP MOVERS'); sym=ctx.get('symbol') or s.get('primary_symbol') or (d.get('primary_story') or {}).get('symbol','')
    sym=str(sym).upper().replace('USDT','').replace('$','').strip()
    ...
    prompt={'version':'5.1','symbol':sym,'format':fmt,'authoritative_context':ctx,'selected_opportunity':selected,'fresh_news':news_context[:12],'instruction':f'''You are the senior editor of a serious but high-energy Binance Square crypto creator. The frozen primary asset is ${sym}; never substitute another asset.
...
Each candidate must be a finished post, not writing instructions or placeholders. It must contain the actual supplied event/observation, why it matters now, concrete evidence, TradingView-based technical context when applicable, conditional bull and bear scenarios when useful, what to watch next, and exactly one natural question. Keep paragraphs mobile-first. At least two candidates must use a non-question hook such as a surprising fact, contrast, mini-story, myth-vs-data, or open loop. At least one candidate should invite a simple A/B decision. At least one should teach one concrete market-reading idea. At least one should use a concise counterpoint.
...
Hook ideas (do not copy mechanically): {json.dumps(hooks,ensure_ascii=False)}''','selection_criteria':['stop-scroll strength','specificity','evidence density','usefulness','conversation quality','originality','mobile readability','scenario clarity','risk discipline','format diversity','news fidelity','finished-post completeness']}
    p['candidate_generation_4']=prompt; PREFLIGHT.write_text(json.dumps(p,indent=2,ensure_ascii=False),encoding='utf-8'); print(json.dumps({'status':'OK','version':'5.1','format':fmt,'symbol':sym,'candidate_count':5,'news_candidates':len(news_context)},indent=2))
