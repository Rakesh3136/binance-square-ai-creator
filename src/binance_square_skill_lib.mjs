...
export async function api(endpoint, apiKey, body, baseUrl = BASE_URL_V2) {
  ...
  catch { throw new Error(`Binance non-JSON response: HTTP ${res.status} ${res.statusText}; body=${raw.slice(0,500)}`); }
  if (json.code !== "000000") throw new Error(`Binance API error [${json.code}]: ${json.message || "unknown"}`);
  return json.data;
}
...
export async function uploadImage(apiKey, imgPath) {
  ...
  const upload = await fetch(ticket.presignedUrl, {
    method: "PUT", headers: { "Content-Type": contentType }, body: fs.readFileSync(imgPath),
  });
