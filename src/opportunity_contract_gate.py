...
def main():
    ...
    raw_score=selected.get('score', selected.get('selected_score', 0))
    score=num(raw_score,-1)
    # Scores are normalized at the freeze boundary. Allow tiny IEEE-754 drift
    # around 0/100, but reject genuine out-of-range values.
    ...
    OUT.write_text(json.dumps({'generated_at':__import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat(),'version':'1.1','status':'PASS' if not failures else 'BLOCK','selected':selected,'symbol':symbol,'category':category,'lane':lane,'failures':failures,'warnings':warnings,'rules':{'score_range':'0-100 inclusive','score_tolerance':1e-9,'flow_min_confidence':FLOW_MIN_CONFIDENCE,'no_unanchored_news_assets':True,'flow_requires_trigger_invalidation_tp1_tp2':True,'conditional_not_predictive':True}},indent=2,ensure_ascii=False),encoding='utf-8')
    if failures:
        print(json.dumps({'status':'BLOCK','failures':failures,'warnings':warnings},indent=2,ensure_ascii=False));raise SystemExit(1)
    print(json.dumps({'status':'PASS','version':'1.1','symbol':symbol,'category':category,'warnings':warnings},indent=2,ensure_ascii=False))
