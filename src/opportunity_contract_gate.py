...
PREFLIGHT=ROOT/'data/live/editorial_preflight.json'; FLOW=ROOT/'data/live/capital_flow_intelligence.json'; OUT=ROOT/'data/live/opportunity_contract.json'
...
def main():
    pre=load(PREFLIGHT);flow=load(FLOW);selected=pre.get('selected_opportunity') or {};director=pre.get('content_director_4') or {}
    category=str(selected.get('category') or '').lower();lane=str(selected.get('lane') or '').lower();symbol=norm(selected.get('symbol') or (director.get('primary_story') or {}).get('symbol'));failures=[];warnings=[]
    ...
    score=num(selected.get('score',selected.get('selected_score',0)),-1)
    ...
    story=director.get('primary_story') or {};chart_symbols=[norm(x) for x in story.get('chart_symbols',[]) if norm(x)]
    # Meme is a reach/commentary lane and is intentionally chart-optional.
    chart_required=category not in {'breaking_news','news_and_macro','watchlist','crypto_meme'}
    if chart_required and symbol not in chart_symbols and chart_symbols:failures.append('chart_symbol_mismatch')
    OUT.parent.mkdir(parents=True,exist_ok=True)
    result={'generated_at':datetime.now(timezone.utc).isoformat(),'version':'1.2','status':'PASS' if not failures else 'BLOCK','selected':selected,'symbol':symbol,'category':category,'lane':lane,'failures':failures,'warnings':warnings,'rules':{'score_range':'0-100 inclusive','score_tolerance':1e-9,'flow_min_confidence':FLOW_MIN_CONFIDENCE,'no_unanchored_news_assets':True,'flow_requires_trigger_invalidation_tp1_tp2':True,'conditional_not_predictive':True,'crypto_meme_is_chart_optional':True}}
    ...
    print(json.dumps({'status':'PASS','version':'1.2','symbol':symbol,'category':category,'chart_required':chart_required,'warnings':warnings},indent=2,ensure_ascii=False))
