...
def hook_quality(hook,symbol,headline,body):
    ...
    # A hook must carry a concrete subject/relationship, not an unfinished or generic claim.
...
def fallback(symbol,body,recent,headline):
    ...
    candidates=[]
    for s in clean_body[:8]:
        candidates.append(f"The important ${symbol} signal is the mechanism behind this move, not the headline itself: {' '.join(s.split()[:12])}.")
...
def main():
    ...
    if not candidate:
        OUT.write_text(json.dumps({'status':'REPAIR_FAILED','reason':'No distinct hook passed semantic, length and specificity checks','original_hook':hook,'matches':near[:5]},indent=2,ensure_ascii=False),encoding='utf-8')
        # Leave the original draft untouched; the downstream judge remains the hard publication gate.
        print(json.dumps({'status':'REPAIR_FAILED','action':'LEAVE_DRAFT_UNCHANGED','reason':'No safe hook generated'},ensure_ascii=False)); return 0
    lines[0]=candidate; new='\n\n'.join(lines)
    repair={'status':'REPAIRED','original_hook':hook,'new_hook':candidate,'max_recent_similarity':max((similarity(candidate,h) for h in recent),default=0.0),'verified_body_preserved':True,'headline_preserved':headline.lower() in new.lower() if headline else True,'quality_checked':True}
    ...
    rc=run_judge()
    # A repair is never allowed to turn a hard editorial failure into a workflow failure here;
    # the authoritative final gate decides publication. This step's job is safe repair only.
    return 0 if rc else 0
