"""Elite Pre-Publication Judge 1.5.
...
Specificity is measured from observable asset-level and evidence-level signals.
A post should not fail merely because it lacks a large number of uppercase
symbols when it is clearly anchored to one asset and contains concrete facts.
"""
...
def semantic_specificity(text,facts,adv=None):
    """Score concrete asset anchoring, not raw ticker count.

    A strong single-asset post can be highly specific even when it discusses
    only one ticker. Asset identity, concrete measurements, market context and
    research anchoring are therefore scored independently.
    """
    low=text.lower()
...
def main():
    ...
    specificity=semantic_specificity(text,facts,adv)
