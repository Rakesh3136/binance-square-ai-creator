...
def score(s,fmt):
 t=str(s or '').strip(); low=t.lower(); w=len(words(t)); pts={'attention':0,'usefulness':0,'interaction':0,'evidence':0,'originality':0,'safety':0}
 ...
 pts['interaction']=min(20,10+(10 if '?' in t else 3 if any(x in low for x in ['agree','choose','which','what do you think']) else 0))
 pts['evidence']=min(15,7+(8 if any(x in low for x in ['according','data','volume','chart','price','news','reported']) else 0))
 pts['originality']=min(10,5+(5 if not low.startswith(('bitcoin is','the crypto market','today')) else 0))
 ...
 return {'total':total,'breakdown':pts,'risk_flag':risky,'word_count':w}
...
def main():
 ...
 result={'version':'4.0','format':fmt,'candidate_count':len(scored),'winner':scored[0] if scored else None,'candidates':scored,'minimum_publish_score':70,'policy':['Never optimize for clicks alone.','Reject unsupported certainty and fabricated claims.','Prefer evidence-rich interaction over empty engagement bait.','Use performance data as an experiment, not as knowledge of a private platform algorithm.']}
